...
app.all('/luta/:id?', authMiddleware, function(request, response) {
				...
				if(request.param('vs')){ // caso haja um param vs, é porque está desafiando alguém
					...
					if(vs == user.uid){ // caso ele desafie ele mesmo
						...
						Characters.lutas_restantes(personagem._id, function(quant){ // verifica se o jogador possui lutas disponíveis
							if(quant <= 0){ // se não possuir lutas disponíveis
								request.session.erro = (user.locale && user.locale.indexOf('pt') >= 0) ? 'Você já lutou muitas vezes hoje e seu meme está cansado, <br />volte amanhã. Quer mais lutas? <a href="loja">Clique aqui</a>' : 'You\'ve fought too many times today and you meme is tired, <br />come back tomorrow. Want more? <a href="loja">Clique here</a>';
					...
					Luta.findOne({_id: luta_id}, function(err, luta){ // busca a luta
						if(luta == null){ // redireciona para o perfil caso ela não exista
							...
							Personagem.findById(luta.personagem1_id).select('_id avatar nome uid hp genero level meme_src').exec(function(err, p1){

								Personagem.findById(luta.personagem2_id).select('_id avatar nome uid hp genero level meme_src').exec(function(err, p2){
