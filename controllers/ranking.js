...
app.all('/ranking', authMiddleware, function(request, response) {
	...
	amigos_usando(request, response, function(result){
		...
		Personagem.where('uid').in(amigos_uids).sort('-level').select('nome avatar uid ranking_pos level').limit(10).exec(function(err, amigos){
			// seleciona os top 10 personagens
			Personagem.where().sort('-level').select('nome avatar uid ranking_pos level meme_src').limit(10).exec(function(err, jogadores){
